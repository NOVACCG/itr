...
# -------------------------
def compute_filled(preset: dict, sheet_name: str, row_dict: Optional[dict], col_map_norm: Optional[dict],
...
def search_label_rect(page: fitz.Page, field: dict) -> List[fitz.Rect]:
    ...
    for r in found:
        ...
        if all((round(u.x0, 1), round(u.y0, 1), round(u.x1, 1), round(u.y1, 1)) != key for u in uniq):
            uniq.append(r)
    return uniq
...
def write_one_itr(doc: fitz.Document, set_start_page_1based: int, preset: dict,
                  filled: Dict[str, str], field_rect_cache: dict):
...
class UniversalITRApp(tk.Tk):
    ...
    def _build_presets_tab(self):
        ...
        ttk.Button(btns, text="使用说明", command=self.open_help).pack(fill="x", pady=6)
...
"""
    ...
    def _read_fields_tree(self) -> List[dict]:
        ...
        for iid in self.field_tree.get_children():
            name, pdf_label, side, page_scope, source, excel_col_norm, const_val, rule = self.field_tree.item(iid,
                                                                                                              "values")
            scope_list = []
    ...
    def _collect_preset_from_editor(self) -> Tuple[str, dict]:
        ...
        d["match"]["excel_key_col_candidates_norm"] = [x.strip() for x in self.ent_key_cols.get().split(",") if
                                                       x.strip()]
    ...
    def _build_main_tab(self):
        ...
        ttk.Button(bottom, text="打开输出文件夹", command=self.open_output_folder).pack(side="right", padx=10)
    ...
    def pick_excel(self):
        path = filedialog.askopenfilename(title="选择 Excel",
    ...
    def pick_pdfs(self):
        paths = filedialog.askopenfilenames(title="选择 PDF（可多选）",
    ...
    def gui_fuzzy_chooser(self, key_pdf: str, short_key: str, candidates: List[str]) -> str:
        ...
        ttk.Checkbutton(win, text="记住该映射（下次遇到同样短Key不再提示）", variable=remember_var).pack(anchor="w",
                                                                                                       padx=10,
                                                                                                       pady=(0, 10))
    ...
    def _preview_worker(self, preset: dict):
        ...
        for pdf_path in self.pdf_paths:
            ...
            self._preview_q.put(("progress", done, total_pdfs, f"解析中：{pdf_name}"))
            doc = fitz.open(pdf_path)
            ...
            for s in valid_starts:
                ...
                status, excel_key, sheet, payload, short_key = match_one(
                    key_pdf, excel_index, preset, self.match_memory, chooser_func=self._thread_fuzzy_chooser
                )

                if payload is None:
                    filled = compute_filled(preset, "", None, None, pdf_name)
                    item = ITRItem(pdf_file=pdf_name, set_start_page_1based=s, key_pdf=key_pdf,
                                   match_status=status, excel_key=excel_key, sheet_name="", filled=filled)
                else:
                    sheet_name, row_dict, col_map_norm = payload
                    filled = compute_filled(preset, sheet_name, row_dict, col_map_norm, pdf_name)
                    item = ITRItem(pdf_file=pdf_name, set_start_page_1based=s, key_pdf=key_pdf,
                                   match_status=status, excel_key=excel_key, sheet_name=sheet_name, filled=filled)
                items.append(item)
    ...
    def _poll_preview_queue(self):
        try:
            while True:
                ...
                kind = msg[0]
                if kind == "progress":
                    done, total, text = msg[1], msg[2], msg[3]
                    ...
                    for i, it in enumerate(self.items):
                        txt = f"{i + 1:03d} | {it.pdf_file} | start p{it.set_start_page_1based:>3} | key={it.key_pdf} | {it.match_status} | {it.excel_key}"
    ...
    def render_current_item(self):
        ...
        active = self.global_cfg.get("active_preset", "")
        preset = load_preset(active) if active else None
        order = [f.get("name", "") for f in preset.get("fields", []) if f.get("name")] if preset else list(
            it.filled.keys())
        for k in order:
        ...
        try:
            ...
            for pdf_path in self.pdf_paths:
                ...
                if not group:
                    ...
                    self._q.put(("progress", done, total_pdfs, f"跳过：{pdf_name}（无ITR项）"))
                    continue

                self._q.put(("progress", done, total_pdfs, f"处理中：{pdf_name}"))
                doc = fitz.open(pdf_path)
                cache = {}
                for it in group:
    ...
    def _save_report(self, preset: dict, report_dir: str) -> str:
        ...
        rows = []
        for it in self.items:
            r = {"PDF": it.pdf_file, "StartPage": it.set_start_page_1based, "PDF_Key": it.key_pdf,
                 "MatchStatus": it.match_status, "ExcelKey": it.excel_key, "Sheet": it.sheet_name}
            empties = []
