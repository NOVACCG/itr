...
def find_cell_by_exact_norm(page: fitz.Page, target: str, verticals, horizontals, search_clip: fitz.Rect | None = None):
    ...
    uniq = []
    for c in candidates:
        if all(not (abs(c.x0 - u.x0) < 1 and abs(c.y0 - u.y0) < 1 and abs(c.x1 - u.x1) < 1 and abs(c.y1 - u.y1) < 1)
               for u in uniq):
            uniq.append(c)
...
def open_folder(path: Path):
    try:
        ...
        messagebox.showinfo("提示", f"文件夹路径：\n{path}")


class NAChkTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Button(top, text="打开输出文件夹 output", command=lambda: open_folder(self._module_root())).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Button(top, text="打开测试输出 test", command=lambda: open_folder(self._module_root() / "test")).pack(side=tk.LEFT, padx=(0, 8))
        ...
        r = 0
        ttk.Radiobutton(cfg, text="正则方式（优先）", variable=self.page_mode, value="regex").grid(row=r, column=0, sticky="w")
        ...
        ttk.Radiobutton(cfg, text="手动方式", variable=self.page_mode, value="manual").grid(row=r, column=0, sticky="w", pady=(6, 0))
        ...
        r += 1
        ttk.Label(cfg, text="底部矩阵表序号表头候选（逗号分隔）：").grid(row=r, column=0, sticky="w", pady=(10, 0), columnspan=2)
        ttk.Entry(cfg, textvariable=self.num_header_var).grid(row=r, column=2, sticky="we", padx=5, pady=(10, 0), columnspan=3)
            ...
            while True:
                ...
                if kind == "log":
                    ...
                    if parsed_pdf_count == 0:
                        messagebox.showwarning("解析失败", "选中范围内没有任何 PDF 解析成功。\n请检查页数配置/NO候选或PDF结构。")
                    else:
                        messagebox.showinfo("解析完成", f"解析完成：PDF={parsed_pdf_count} 个，命中 ITR={total_itr_ok} 套。\n现在可点【测试】。")
                    return
                elif kind == "test_done":
                    out_dir = msg[1]
    ...
    def _update_status(self):
        ...
        self.status.config(text=f"已导入PDF：{len(self.pdf_paths)}（可多选） | 选中：{len(sel) if sel else 0 if self.pdf_paths else 0}{'（默认全部）' if (self.pdf_paths and not sel) else ''}")
    ...
    def _parse_worker(self, targets: list[str], num_candidates: list[str]):
        ...
        for pdf_path in targets:
            ...
            for i in range(itr_count):
                ...
                if not no_cell:
                    parsed_list.append({
                        "itr": i + 1, "page_index": pidx,
                        "no_cell": None, "band": None,
                        "desc_cell": None, "ex_cells": [],
                        "okna": {}, "ex_label": None, "ex_value": None,
                    })
                    continue
                ...
                parsed_list.append({
                    "itr": i + 1, "page_index": pidx,
                    "no_cell": no_cell, "band": band,
                    "desc_cell": desc_cell, "ex_cells": ex_cells,
                    "okna": okna, "ex_label": ex_label, "ex_value": ex_value,
                })
                ok_count += 1
    ...
    def test_selected(self):
        ...
        if not_ok:
            messagebox.showwarning("提示", f"以下 PDF 尚未解析成功，无法测试：\n" + "\n".join(Path(p).name for p in not_ok) + "\n\n请先点【解析】。")
            return
    ...
    def _tick_worker(self, targets: list[str], out_dir: Path, report_dir: Path, total_pages: int):
        ...
        for pdf_path in targets:
            ...
            for entry in parsed_list:
                ...
                x_right = max(no_rect.x1, target_ex_rect.x1, na_hdr.x1,
                              (ok_hdr.x1 if ok_hdr else 0), (pl_hdr.x1 if pl_hdr else 0))
...
def run_standalone():
    root = tk.Tk()
    root.title("ITR 工具 - NA 自动勾选（正式Tab v3.4 批量PDF）")
    ...
    tab1 = ttk.Frame(nb)
    ttk.Label(tab1, text="（占位）这里模拟你的主程序其它 Tab").pack(padx=20, pady=20)
    nb.add(tab1, text="主程序占位")

    tab2 = NAChkTab(nb)
    nb.add(tab2, text="NA 自动勾选")
